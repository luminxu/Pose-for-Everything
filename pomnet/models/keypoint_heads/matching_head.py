...
class RelationModel(nn.Module):
    ...
    def forward(self, feature_s, target_s, feature_q):
        """Forward function."""
        pooled_feature_list = []
        for feature, target in zip(feature_s, target_s):
            resized_feature = resize(
                ...
                mode='bilinear',
                align_corners=False)
            pooled_feature = self.pool(resized_feature * target)
            pooled_feature_list.append(pooled_feature)
        pooled_feature_s = torch.mean(torch.stack(pooled_feature_list, dim=0), 0)
...
class MatchingHead(TopdownHeatmapBaseHead):
    """Matching Head
    ...
    def __init__(self,
        ...
        self.relation_model = RelationModel(in_channels*2, in_channels)
    ...
    def forward(self, feature_s, target_s, feature_q):
        """Forward function."""
        # feature_s = self._transform_inputs(feature_s)
        feature_q = self._transform_inputs(feature_q)

        feature = self.relation_model(feature_s, target_s, feature_q)
